...
void Setting()
{
    ...
    {
        sprintf(buf, SETTING, g_cfg.ssid.c_str(), g_cfg.password.c_str(),g_cfg.tianqi_appid.c_str(),
                g_cfg.bili_uid.c_str(), g_cfg.tianqi_appsecret.c_str(), g_cfg.backLight, 
                g_cfg.rotation,  "checked=\"checked\"", "");
    }
    else
    {
        sprintf(buf, SETTING, g_cfg.ssid.c_str(), g_cfg.password.c_str(), g_cfg.tianqi_appid.c_str(), 
                g_cfg.tianqi_appsecret.c_str(), g_cfg.bili_uid.c_str(), g_cfg.backLight, 
                g_cfg.rotation, "", "checked=\"checked\"");
    }
    webpage = buf;
    Send_HTML(webpage);
