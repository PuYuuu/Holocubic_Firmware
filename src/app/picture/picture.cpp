...
void picture_init(void)
{
    ...
    run_data->picRefreshInterval = 18000;
    ...
    char* _startP = run_data->pfile->file_name;
    int _maxFind = 20, count = 0;
    do {
        if (strstr(run_data->pfile->file_name, "sly_1") != NULL) {
            break;
        } else {
            run_data->pfile = run_data->pfile->next_node;
            count++;
        }
    } while(strcmp(run_data->pfile->file_name, _startP) && count < _maxFind);
