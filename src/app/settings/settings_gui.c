...
void display_settings(const char *cur_ver, const char *new_ver, lv_scr_load_anim_t anim_type)
{
    ...
    lv_label_set_text(title_label, "#00ff00 Interstellar#");
    ...
    lv_label_set_text(cur_ver_label, "Christopher\tNolan");
    ...
    lv_label_set_text(qq_label, "Gravity&Love");
    lv_obj_align(qq_label, NULL, LV_ALIGN_OUT_BOTTOM_MID, 0, -120);

    lv_obj_add_style(new_ver_label, LV_LABEL_PART_MAIN, &label_style);
    ...
    lv_label_set_text(new_ver_label, "Do not go gentle int");
    lv_label_set_recolor(new_ver_label, true); //先得使能文本重绘色功能
    lv_label_set_long_mode(new_ver_label, LV_LABEL_LONG_SROLL_CIRC);
    lv_label_set_text(new_ver_label, "Do not go gentle into that good night. Rage,rage against the dying of night.  ");
    lv_obj_align(new_ver_label, NULL, LV_ALIGN_OUT_BOTTOM_MID, 0, -80);

    lv_obj_add_style(author_label, LV_LABEL_PART_MAIN, &label_style);
    lv_label_set_recolor(author_label, true); //先得使能文本重绘色功能
    lv_label_set_text(author_label, "@#ffa500 Shi-Li-Yu#");
